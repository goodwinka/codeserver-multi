...
- ✅ Виртуальный GUI-экран per-user (noVNC), доступный из нижней панели
...
## Требования для виртуального GUI

Виртуальный GUI (кнопка «Виртуальный экран») требует системные пакеты внутри контейнера:
`xvfb`, `openbox`, `x11vnc`, `websockify`, `novnc`.

Если вы используете образ, собранный до этой доработки, пересоберите контейнер:
...
## noVNC: почему приходится вручную указывать `/proxy/<port>/`

Это ожидаемое поведение, если открывать noVNC **напрямую** через VS Code Ports (`/proxy/<port>/`).

- `x11vnc`/`websockify` запускаются **лениво** только когда приложение получает запрос на `/_auth/gui-url` или `/_auth/gui/<username>/...`.
- Поэтому до первого такого запроса локальный порт GUI (например, `9719`, `9848`) ещё не слушает.
- Из-за этого при ручном открытии noVNC часто кажется, что сервер «оживает» только после чтения логов (`cat /tmp/x11vnc-<user>.log`), но на самом деле запуск триггерится другим запросом к GUI-роутам.

Рекомендуемый способ:

1. Открывать виртуальный экран через кнопку **«Виртуальный экран»** во встроенном UI (`frame.html`) или через `/_auth/gui-url`.
2. Использовать URL, который вернёт `/_auth/gui-url` (там уже правильно заполнены `host/port/path`).

Если всё же открываете noVNC вручную, путь должен быть именно вида `/proxy/<websockify-port>/` (со слэшем в конце).
