...
## Диагностика noVNC (важно)

В этом проекте noVNC **не** поднимается как отдельный сервис на `127.0.0.1:6080`.
GUI стартует **персонально для пользователя** после логина и проксируется через auth-proxy:

- `/_auth/gui-url` — получить URL GUI для текущей сессии
- `/_auth/gui/<username>/vnc.html` — страница noVNC
- `/_auth/gui/<username>/websockify` — WebSocket endpoint

Поэтому проверка `curl http://127.0.0.1:6080` в этом репозитории обычно вернёт
`connection refused` — это нормально для данной архитектуры.

Если GUI не открывается после логина:

1. Проверьте логи контейнера: `docker compose logs -f`
2. Ищите ошибки `[login] start GUI failed ...` и `[gui:<username>] ...`
3. Убедитесь, что образ пересобран после изменений: `docker compose build --no-cache && docker compose up -d`
